...
public class DummyLocationService {
    ...
    private void parseFile(Context context) {
        locationList.clear();
        // resource reference to dummy_data in res/raw/ folder of your project
        try (Scanner scanner = new Scanner(context.getResources().openRawResource(R.raw.dummy_data))) {
            ...
            while (scanner.hasNext()) {
                ...
                String timeValues[] = scanner.next().split(":| ");
                Calendar time = Calendar.getInstance();
                time.set(Calendar.HOUR, Integer.parseInt(timeValues[0]));
                time.set(Calendar.MINUTE, Integer.parseInt(timeValues[1]));
                time.set(Calendar.SECOND, Integer.parseInt(timeValues[2]));
                if (timeValues[3].equals("AM")) {
                    time.set(Calendar.AM_PM, Calendar.AM);
                } else {
                    time.set(Calendar.AM_PM, Calendar.PM);
                }
                friend.time = time.getTime();
